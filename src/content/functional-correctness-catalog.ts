...
const noCompilationReceipt =
  `The compiler gate is integrated, but no generated Verus report is bound to this lesson's exact compilation. Its generated formula-replay source and result, status-Checked policy staging, PLIRON structural reports, tool identity, and SafeReferenceMirToLivePliron boundary must bind one compilation at the private move-only join. Staging grants no authority alone. Maintainers report that the publication commands passed on mi300x against compiler ${semanticCorrectnessMilestone.compilerCommit} and the pinned Verus executable, but this website retains no immutable transcript or run record for that qualification. The reported compiler and host-proof validation does not bind a generated report to this lesson's exact compilation and grants no target, artifact, launch, hardware, performance, or universal-correctness authority; there is no fallback.`;
