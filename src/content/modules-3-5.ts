...
const gemmMapping: Lesson = {
  ...
  objectives: [
    ...
    "Separate completed one-shot lifecycle mechanics from the still-unmeasured protected hardware execution path.",
  ],
  claims: [
    {
      kind: "design-only",
      ...
      detail:
        "fe2o3 now authenticates the fixed attributed LDS Slice 1 source through canonical V5 Kernel IR into an exact compiler-owned descriptor and single-use Worker V2 handoff, admits that handoff into a sealed authority-free exact-profile registry, finalizes it through direct upstream LLVM target-machine and LLD library APIs, prepares exact borrowed A/B/C views with a generated inert host adapter, and consumes those values through a private one-shot Joined -> Loaded -> Completed -> Unloaded lifecycle with exact context, resource, ABI, completion, and terminal-unload checks. It proves a bounded identity-bound Slice 1 source/model relation, has independent Slice 2 proof and K32 backend records, and lowers and inspects exact Slice 3 and tail-safe Slice 4 graphs through upstream LLVM/COV6. The lifecycle integration suite uses a fake adapter: real protected hardware measurement, protected-execution authority, compiler refinement, certificate consumption, and generalization remain open, so this is not a functional or production kernel.",
    },
    {
      kind: "compiler-hsaco-observed",
  ...
  tabs: completeTabs(
    ...
    { language: "bash", code: noHost, explanatory: true },
    {
      ...
      code: resultText(
        "design-only",
        "Slice 1 source-to-IR, canonical matrix wire V5, exact compiler descriptor, Worker V2 handoff, sealed exact-profile import, direct LLVM/LLD API finalization, exact generated host preparation, one-shot Joined -> Loaded -> Completed -> Unloaded mechanics, and bounded identity-bound source/model correspondence; Slice 2 proof and K32 backend evidence; and exact Slice 3 and Slice 4 LLVM/COV6 inspection exist as bounded increments. #85, #86, #93, #96, #97, #99, and #100 are complete. The #100 integration evidence uses a fake adapter and contains no real protected hardware measurement; certificate consumption and proof extension (#91/#92), protected Slice 3/4 execution (#88/#89), and generalized GEMM (#90) remain open. No compiler-origin, rustc/LLVM/machine refinement, protected-execution, or production source-to-HSACO authority is claimed.",
      ),
    },
  ),
  diagram: "gemm",
  ...
  claims: [
    {
      ...
      detail:
        "The fixed Slice 1 attributed source reaches canonical V5 Kernel IR, an exact compiler descriptor, an inert final HSACO receipt through direct LLVM/LLD APIs, exact generated host preparation, and the one-shot Joined -> Loaded -> Completed -> Unloaded implementation; its bounded identity-bound source/model relation verifies, and exact Slice 3/4 graphs reach inspected machine shape. Real protected hardware measurement, proof-certificate consumption, Slice 2-4 proof extension, generalized attributed source, protected-execution authority, compiler and machine refinement, and an IEEE numerical contract remain required before promotion.",
    },
  ],
  sections: [
