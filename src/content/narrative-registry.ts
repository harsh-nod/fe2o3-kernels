  ...
  "read-the-evidence/scalar-gemm-checkpoint": {
    ...
    "blocks": [
      ...
      },
      {
        ...
        "text": "The checked-in publication gate names public-main implementation commit 38b0005765944de55bb32c559bc8431637317b2b, tree adf58f5a6c0d9b9a6d32a9d69ef54dbb48d9ec99, while lesson claim badges remain pinned to FE2O3_PIN until a separate baseline audit. Both harsh-nod/fe2o3@refs/heads/main and powderluv/fe2o3@refs/heads/main resolve exactly to that descendant, and deployment continues to require that exact match. The protected implementation and measured evidence remain pinned to c4fcb4d980cf979c0527dfa135a7b9f4fe72a811, tree c65c6ab567409afaaef6ea39c8befcac21d47119. The exact Wave64 and synchronization source snapshots remain pinned separately to d592ecee1154ca39daf1f9b1c2e02ab462e6c5f8, tree cdec8448a300aa71d17565ca50fd4d893932f602. The newer public snapshot contains Worker V2 metadata hardening, shared GEMM and row-softmax numerical contracts, source/oracle/formal packages for masked Wave64 collectives and workgroup synchronization, typed DeviceGlobalMutPtr plus exclusive generated GlobalMut host admission, an exact linear DynamicLds capability, the trust-pin repair, root status documentation, cross-process CI broker fixture isolation, a 64-connection capacity test isolated from the separate concurrent authentication and descriptor-transfer test, refreshed standalone Wave 2 dependency locks, and the row-softmax V1 certificate-authentication, exact finalizer join, sealed host-token stages, generated typed 64-element host adapter, and private linear HSA load/dispatch/wait/unload lifecycle. The Wave64 profile now additionally authenticates exact attributed source, FnAbi, trusted definitions, complete reachable portable MIR, gfx942/COV6/Wave64 geometry, mask semantics, ordered collectives, and lane-owned outputs into a closed semantic Kernel IR sidecar; it still has no LLVM lowering, finalizer, generated host/runtime launch, protected artifact, or hardware authority. The synchronization sources still stop before compiler collection and Kernel IR selection. The row-softmax lifecycle authenticates the exact gfx942:xnack- target, COV6 symbol, 288-byte kernarg, resource envelope, and launch geometry and compiles a fail-closed protected hardware harness. No new protected execution is claimed because the MI300X environment supplied 0 of the 25 exact compiler, worker, and OCML pins required by that harness. None of the newer increments retroactively strengthens the bounded protected observation. The typed staged records do not combine authenticated compiler origin, compiler refinement, final machine semantics, and generalized safety into one authority chain."
      }
    ]
  },
...
const reviewedNarrativeFingerprints = deepFreeze({
  ...
  "read-the-evidence/scalar-gemm-checkpoint":
    "68e678bef19943e810824bd72a609277e55e9dc2b62e5f69faeb35ef4d73bcf4",
  "reductions-scans/scan":
