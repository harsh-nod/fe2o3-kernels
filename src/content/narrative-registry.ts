...
const narrativeRegistry = deepFreeze({
  ...
  "cpu-semantic-simulation/pipeline": {
    ...
    "blocks": [
      ...
      {
        "type": "paragraph",
        "text": "The public 2df6130c5 scan milestone keeps its API, three production LLVM representatives, and six direct KIR V10 semantic cases as independent historical layers. At b15cf628f, a separate gfx942 gate exports all 18 ordinary source entries across u32, i32, f32, inclusive/exclusive mode, and extents 3, 65, and 255 as authority-free Bundle V5. Their trap-bearing Semantic MIR uses additive V11 while V10 stays byte-for-byte closed; production KIR V8 maps to exact same-module KIR V10. The direct CPU simulator, complete Trace V2, SimRuntimeBackendV1, and canonical persisted seeded-schedule replay match exact output; the JSONL debugger separately checks bounded stops and nonempty hierarchy views. N=65 exposes the one-lane final logical Wave64; N=255 stops inexactly at the debugger resource boundary while its retained prefix remains inspectable. The generated bundles and schedules remain ephemeral test artifacts, and neither layer is GPU execution or performance prediction."
      },
      {
        "type": "table",
        "headers": [
        ...
        "rows": [
          ...
          [
            ...
            "Formal memory, exact scalar bits, cooperative barriers, integer atomics and fences, compiler-projected aggregate leaves, and a bounded runnable-invocation decision record. Bundle V5 runs ordinary recursive aggregates from production KIR V8 and the gfx950 f32 wave-reduction body from production KIR V9 as exact same-module KIR V10; commit 62965b3f8 separately executes explicitly sized dynamic LDS in direct KIR V10. At 2df6130c5, direct KIR V10 executes all six admitted workgroup scan type/mode pairs. At b15cf628f, all 18 ordinary scan sources select additive Semantic MIR V11 and execute through Bundle V5 plus exact persisted seeded replay.",
            "Legacy Bundle V4 remains canonical KIR V7. The exact canonical compiler bounds trap is a dynamic terminal error only when reached; other external diagnostics remain typed unavailable. Unsupported operations, missing dynamic-LDS size, changed request or artifact custody, transcript drift, and static launch mismatch fail before replay is accepted. N=255 intentionally demonstrates a bounded, inexact resource-exhaustion stop rather than silently presenting a truncated debugger transcript as complete."
          ]
        ]
      },
      {
