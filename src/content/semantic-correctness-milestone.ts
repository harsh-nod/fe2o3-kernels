...
export const semanticMilestoneBoundary =
  semanticCorrectnessMilestone.status === "integration-pending"
    ...
    : `Milestone status: ${semanticCorrectnessMilestone.status} at compiler ${semanticCorrectnessMilestone.compilerCommit}, pre-lowering report ${semanticCorrectnessMilestone.preloweringReportVersion}, and functional receipt ${semanticCorrectnessMilestone.functionalReceiptVersion}. Read each capability status and claim for its remaining source-to-machine, termination, arithmetic-definedness, and numeric boundary.`;
