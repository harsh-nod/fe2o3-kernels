/** Explicit V21 complete recording presentation. No V20 fallback or live command route. */
import { parseProgramJson, programSha256 } from "./ordered-program-observation.mjs";
import { anchor, memory, snapshot, stop } from "./physical-entry-debug-v20-views";
import { capabilities, cursor, digest, freeze, need, object, request as requestShape, same, session, stable, uint,
  type PhysicalObservationV20, type Row } from "./physical-entry-debug-v20-shapes";
import { valuesV21 } from "./physical-global-copy-debug-v21-views";
import { auditCopyRecordingV21 } from "./physical-global-copy-debug-v21-audit";
import { copyContextV21, PHYSICAL_COPY_LIMITS_V21 as LIMITS,
  type PhysicalCopyInputV21, type PhysicalCopyProjectionV21 } from "./physical-global-copy-debug-v21-input";
export { PHYSICAL_COPY_SELECTOR_V21, PHYSICAL_COPY_SCHEMA_V21, PHYSICAL_COPY_LIMITS_V21 } from "./physical-global-copy-debug-v21-input";
export type { PhysicalCopyInputV21, PhysicalCopyProjectionV21, PhysicalCopyRecordingV21 } from "./physical-global-copy-debug-v21-input";
function request(value: unknown): Row {
  try { return requestShape(value); } catch { throw new Error("Invalid or unsupported V21 recorded command."); }
}
...
function errorResult(r: Row, q: Row, previous: Row, end: number): string {
  ...
  if (q.expected_revision !== previous.revision) {
    same(error.code, "stale_revision"); same(error.message, "kir_v21_debug_stale_revision");
    ...
    same(error.message, foreign ? "kir_v21_debug_foreign_or_stale_cursor" : "kir_v21_debug_cursor_out_of_range");
  } else {
    need((error.code === "resource_limit" && ["kir_v21_debug_revision_limit", "kir_v21_debug_navigation_unavailable"].includes(String(error.message))) ||
      (error.code === "response_too_large" && error.message === "kir_v21_debug_response_too_large"), "Unknown or contradictory error response.");
  }
...
function unavailableResult(r: Row, q: Row, previous: Row, currentAnchor: Row | null): string {
  ...
  same(value.state_changed, false); same(value.detail, "diagnostic physical-global-copy V21 exposes bounded CPU observations only");
...
export async function projectPhysicalCopyDebugV21(input: PhysicalCopyInputV21 | null): Promise<PhysicalCopyProjectionV21> {
  if (input === null) return { status: "unavailable", detail: "No V21 recording selected. No previous values are shown." };
  try {
    const context = await copyContextV21(input);
    const requestLines = lines(context.requestsUtf8, LIMITS.requestBytes), responseLines = lines(context.responsesUtf8, LIMITS.responseBytes);
    ...
    const [requestSha256, responseSha256] = await Promise.all([programSha256(context.requestsUtf8), programSha256(context.responsesUtf8)]);
    need(responses.some(r => (r.status === "error" && typeof (r.error as Row)?.message === "string" &&
      String((r.error as Row).message).startsWith("kir_v21_debug_")) || (r.status === "unavailable" &&
      (r.unavailable as Row)?.detail === "diagnostic physical-global-copy V21 exposes bounded CPU observations only")),
      "Recording has no explicit V21 diagnostic profile marker.");
    const end = endSequence(responses), first = requests[0]; same(end, context.records + 1);
    ...
    same(configuration, context.configuration);
    let valueRows = 0, memoryCells = 0;
    ...
    for (let i = 0; i < requests.length; i++) {
      ...
      if (r.status !== "ok") {
        ...
        if (operation === "step" || operation === "seek") {
          ...
          same(view, previous);
          if (operation === "discover_capabilities") { capabilities(result); display = { ...record, detail: "Recorded V21 CPU capability roster." }; }
            ...
            if (operation === "inspect_values") {
              need(Array.isArray(result.values) && result.values.length <= LIMITS.page, "SSA page bound exceeded.");
              valueRows += result.values.length; need(valueRows <= LIMITS.valueRows, "Decoded SSA row budget exceeded.");
              const value = await valuesV21(result, q, view);
              ...
              const returnedBytes = uint((result.memory as Row | undefined)?.returned_bytes, LIMITS.memoryBytes);
              memoryCells += returnedBytes; need(memoryCells <= LIMITS.memoryCells, "Decoded memory-cell budget exceeded.");
    ...
    const facts = auditCopyRecordingV21(context, records);
    return freeze({ status: "ready", key: context.containerSha256, configuration, requestSha256, responseSha256, records, context, facts });
  } catch (error) {
    return { status: "invalid", detail: "V21 recording refused; no partial or previous data displayed. " +
