...
const sourceMilestoneRecords = deepFreeze({
  ...
  "tiled-gemm-safe-source-v1": {
    ...
    commit: "24f666bb01a809cdf985297eef5b3a276f5083d8",
    tree: "8828e6b0b410399c7e3fa46df61ca953dafa157b",
    commands: [
    ...
    primarySourceSha256:
      "eab0ec261d3ce83813ec98c0d06eba40255bfc4d080b3e182a2e9a3ee8c3c78d",
    target: "gfx942:xnack-",
  },
  "wave64-collectives-source-v1": {
  ...
  "reference-refinement-v1": {
    id: "reference-refinement-v1",
    lessonId: "gemm-tiling",
    claim: "source-model-verified",
    authority: "source-model-only",
    claimLabel: "Workload-neutral hierarchy refinement theorem",
    detail:
      "Pinned Verus proves a workload-neutral composition theorem: when every logical output coordinate has one hierarchy owner and that owner's write equals the safe sequential reference value, the complete GPU output equals the CPU reference. Wrong-value and duplicate-owner mutations are rejected. The production PLIRON route now requires an exact reference obligation, checks semantic-expression equality, and runs bounded invocation, subgroup, workgroup, and grid ownership analysis. The compiler does not yet authenticate a Verus certificate or establish full Rust-source-to-ISA refinement, so this record grants source-model authority only.",
    commit: "af0fd523e3b774377a9c5192cf0511e34fa19735",
    tree: "37ec6083aba26f3057bb21f3a51c619c17bceb49",
    commands: [
      "cargo test --locked --manifest-path examples/verus_vecadd/Cargo.toml",
      "VERUS=/absolute/path/to/pinned/verus examples/verus_vecadd/run-verus.sh --require",
      "cargo test -p dialect-kernel -p dialect-proof -p fe2o3-kernel-analysis -p fe2o3-pliron --locked",
    ],
    sourcePaths: [
      "examples/verus_vecadd/src/reference.rs",
      "examples/verus_vecadd/verus/reference_refinement_v1.rs",
      "examples/verus_vecadd/verus/negative/reference_refinement_wrong_value.rs",
      "examples/verus_vecadd/verus/negative/reference_refinement_duplicate_owner.rs",
      "examples/verus_vecadd/run-verus.sh",
      "crates/fe2o3-kernel-analysis/src/pliron_semantic_refinement.rs",
      "crates/fe2o3-kernel-analysis/src/pliron_hierarchical_ownership.rs",
