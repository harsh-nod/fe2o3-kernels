...
# Builds one ordinary Rust FP8 GEMM through the production extractor, proves
...
OUTPUT_ROOT=${FE2O3_GFX950_FP8_OUTPUT_DIR:-$SCRIPT_DIR/target/fe2o3-gfx950-fp8-gemm}
...
LLVM_IR=$ATTEMPT_DIR/gfx950-fp8-gemm.ll
OBJECT=$ATTEMPT_DIR/gfx950-fp8-gemm.o
HSACO=$ATTEMPT_DIR/gfx950-fp8-gemm.hsaco
DISASSEMBLY=$ATTEMPT_DIR/gfx950-fp8-gemm.isa
NOTES=$ATTEMPT_DIR/gfx950-fp8-gemm.notes
...
(
    ...
    LD_LIBRARY_PATH="$ROOT_TARGET_DIR/debug/deps:$SYSROOT/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
        ...
        --no-default-features --features kernel-fp8-gemm --lib
)
...
require_llvm_line_count 'define amdgpu_kernel void @gfx950_fp8_gemm_rust(' 1 \
    'gfx950_fp8_gemm_rust definition'
...
for required in \
    ...
    '^[[:space:]]*\.name:[[:space:]]+gfx950_fp8_gemm_rust[[:space:]]*$' \
    '^[[:space:]]*\.symbol:[[:space:]]+gfx950_fp8_gemm_rust\.kd[[:space:]]*$'; do
...
KERNEL_ISA=$(awk '
    /<gfx950_fp8_gemm_rust>:/ { capture = 1 }
    capture && /^[[:xdigit:]]+ <[^>]+>:/ && $0 !~ /<gfx950_fp8_gemm_rust>:/ { exit }
    capture { print }
' "$DISASSEMBLY")
if [[ -z $KERNEL_ISA ]]; then
    printf 'ISA validation failed: gfx950_fp8_gemm_rust is absent\n' >&2
    exit 1
fi
MFMA_COUNT=$(grep -Fc -- 'v_mfma_f32_16x16x128_f8f6f4' <<< "$KERNEL_ISA" || true)
if [[ $MFMA_COUNT -ne 1 ]]; then
    printf 'ISA validation failed: expected one gfx950 scaled FP8 MFMA, found %s\n' \
        "$MFMA_COUNT" >&2
    exit 1
fi
for forbidden in v_cvt_f32_fp4 v_cvt_f32_fp8 v_fma_f32 v_dot; do
...
(
    ...
    FE2O3_GFX950_FP8_GEMM_HSACO=$HSACO \
    FE2O3_GFX950_FP8_GEMM_SHA256=$HSACO_SHA256 \
    CARGO_TARGET_DIR=$ROOT_TARGET_DIR \
...
printf 'PASS fe2o3 gfx950 FP8 GEMM production build and numerical run\n'
printf 'LLVM:  %s\n' "$LLVM_IR"
printf 'HSACO: %s\n' "$HSACO"
printf 'SHA256: %s\n' "$HSACO_SHA256"
printf 'ISA:   %s\n' "$DISASSEMBLY"
