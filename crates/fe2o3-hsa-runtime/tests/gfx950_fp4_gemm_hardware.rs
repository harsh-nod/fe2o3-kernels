//! Non-authoritative gfx950 FP4 GEMM hardware and numerical harness.
//!
...
//! evidence. Its purpose is to exercise the first Rust-produced gfx950 FP4
...
#[cfg(feature = "hardware-test-hooks")]
const KERNEL_EXPORT: &str = "gfx950_fp4_gemm_rust";
#[cfg(feature = "hardware-test-hooks")]
const KERNEL_DESCRIPTOR: &str = "gfx950_fp4_gemm_rust.kd";
#[cfg(feature = "hardware-test-hooks")]
const M: usize = 16;
#[cfg(feature = "hardware-test-hooks")]
const N: usize = 16;
...
fn parse_sha256(text: &str) -> Result<[u8; 32], BoxError> {
    require(
        ...
        "FE2O3_GFX950_FP4_GEMM_SHA256 must be exactly 64 lowercase hex digits",
    )?;
    let mut bytes = [0; 32];
    for (index, byte) in bytes.iter_mut().enumerate() {
        *byte = u8::from_str_radix(&text[index * 2..index * 2 + 2], 16)
            .map_err(|_| "FE2O3_GFX950_FP4_GEMM_SHA256 is malformed")?;
    }
    Ok(bytes)
}
...
fn read_pinned_hsaco() -> Result<(Vec<u8>, PayloadDigest), BoxError> {
    require(
        std::env::var("FE2O3_RUN_GFX950_FP4_GEMM_HARDWARE").as_deref() == Ok("1"),
        "set FE2O3_RUN_GFX950_FP4_GEMM_HARDWARE=1 to opt into this raw hardware test",
    )?;
    let path = std::path::PathBuf::from(
        std::env::var_os("FE2O3_GFX950_FP4_GEMM_HSACO")
            .ok_or("FE2O3_GFX950_FP4_GEMM_HSACO is not set")?,
    );
    require(
        path.is_absolute(),
        "FE2O3_GFX950_FP4_GEMM_HSACO must be an absolute path",
    )?;
    let metadata = std::fs::symlink_metadata(&path)?;
    require(
        metadata.file_type().is_file() && !metadata.file_type().is_symlink(),
        "FE2O3_GFX950_FP4_GEMM_HSACO must name a regular non-symlink file",
    )?;
    require(
        std::fs::canonicalize(&path)? == path,
        "FE2O3_GFX950_FP4_GEMM_HSACO must already be canonical",
    )?;
    require(
        (1..=fe2o3_hsaco::MAX_HSACO_BYTES as u64).contains(&metadata.len()),
        "FE2O3_GFX950_FP4_GEMM_HSACO has an invalid byte length",
    )?;
    let expected = parse_sha256(
        &std::env::var("FE2O3_GFX950_FP4_GEMM_SHA256")
            .map_err(|_| "FE2O3_GFX950_FP4_GEMM_SHA256 is not set")?,
    )?;
    let bytes = std::fs::read(&path)?;
    require(
        bytes.len() as u64 == metadata.len(),
        "FE2O3_GFX950_FP4_GEMM_HSACO changed size while being read",
    )?;
    ...
    require(
        final_metadata.file_type().is_file()
            ...
            && std::fs::canonicalize(&path)? == path,
        "FE2O3_GFX950_FP4_GEMM_HSACO changed identity while being read",
    )?;
    let digest = DigestAlgorithm::Sha256.calculate(&bytes);
    require(
        digest.bytes().as_bytes() == &expected,
        "FE2O3_GFX950_FP4_GEMM_HSACO does not match its exact SHA-256 pin",
    )?;
    Ok((bytes, digest))
}
...
fn inspect_profile(bytes: &[u8]) -> Result<(), BoxError> {
    ...
    require(
        ...
        "gfx950 FP4 GEMM HSACO must be code object V6",
    )?;
    require(
        inspected.target().processor() == "gfx950"
            && inspected.target().xnack() == Some(FeatureState::Disabled),
        "gfx950 FP4 GEMM HSACO must target exact gfx950:xnack-",
    )?;
    require(
        !inspected.has_printf_metadata(),
        "gfx950 FP4 GEMM HSACO must not carry printf metadata",
    )?;
    let [kernel] = inspected.kernels() else {
        return Err("gfx950 FP4 GEMM HSACO must declare exactly one kernel".into());
    };
    require(
        kernel.name() == KERNEL_EXPORT && kernel.symbol() == KERNEL_DESCRIPTOR,
        "gfx950 FP4 GEMM HSACO has a substituted kernel or descriptor symbol",
    )?;
    require(
        ...
        "gfx950 FP4 GEMM HSACO does not expose the exact 48-byte explicit kernarg",
    )?;
    require(
        ...
        "gfx950 FP4 GEMM HSACO does not expose the exact static WG64 profile",
    )?;
    ...
    require(
        ...
        "gfx950 FP4 GEMM HSACO has a substituted three-slice ABI",
    )?;
    let [binding] = bound.bindings() else {
        return Err("gfx950 FP4 GEMM HSACO must bind exactly one descriptor".into());
    };
    ...
    require(
        ...
        "gfx950 FP4 GEMM descriptor disagrees with its metadata",
    )
}
...
fn deterministic_inputs() -> (Vec<u8>, Vec<u8>) {
    const CODES: [u8; 16] = [
        0x0, 0x1, 0x9, 0x2, 0xa, 0x3, 0xb, 0x4, 0xc, 0x5, 0xd, 0x6, 0xe, 0x7, 0xf, 0x8,
    ];
...
fn cpu_reference(lhs: &[u8], rhs: &[u8]) -> Vec<f32> {
    ...
    for row in 0..M {
        for column in 0..N {
            let mut value = 0.0;
            for depth in 0..K {
                value += decode_fp4_e2m1(lhs[row * K + depth])
                    * decode_fp4_e2m1(rhs[depth * N + column]);
            }
...
fn f32_values(bytes: &[u8]) -> Result<Vec<f32>, BoxError> {
    require(
        ...
        "gfx950 FP4 GEMM output contains a partial f32",
    )?;
    Ok(bytes
        .chunks_exact(4)
...
impl RuntimeKernarg {
    fn new() -> Result<Self, BoxError> {
        ...
        let pointer = std::ptr::NonNull::new(unsafe { std::alloc::alloc_zeroed(layout) })
            .ok_or("failed to allocate aligned gfx950 FP4 GEMM kernarg")?;
...
unsafe fn dispatch_one_wave(
    ...
    require(
        ...
        "runtime resolution differs from the exact gfx950 FP4 GEMM ABI",
    )?;
...
fn output_body_address(buffer: &ReviewedHsaHardwareTestBufferV1) -> Result<u64, BoxError> {
    require(
        ...
        "guarded gfx950 FP4 GEMM output has the wrong extent",
    )?;
...
fn verify_output(actual: &[f32], expected: &[f32]) -> Result<(), BoxError> {
    require(
        ...
        "guarded gfx950 FP4 GEMM output length changed",
    )?;
    ...
    require(
        ...
        "gfx950 FP4 GEMM output prefix canary changed",
    )?;
    require(
        suffix
            .iter()
            .all(|value| value.to_bits() == C_SUFFIX.to_bits()),
        "gfx950 FP4 GEMM output suffix canary changed",
    )?;
    require(
        expected.len() == C_ELEMENTS,
        "gfx950 FP4 GEMM CPU reference has the wrong extent",
    )?;
    ...
    for (index, (actual, expected)) in body.iter().zip(expected).enumerate() {
        ...
        require(
            ...
            format!(
                "gfx950 FP4 GEMM C[{index}] mismatch: actual={actual}, expected={expected}, tolerance={MAX_ABSOLUTE_ERROR}"
            ),
        )?;
        maximum_absolute_error = maximum_absolute_error.max(absolute_error);
    }
    println!(
        "PASS gfx950 FP4 GEMM outputs={C_ELEMENTS} max_absolute_error={maximum_absolute_error:.9e} tolerance={MAX_ABSOLUTE_ERROR:.1e}"
    );
    Ok(())
}
...
fn run_hardware(bytes: Vec<u8>, digest: PayloadDigest) -> Result<(), BoxError> {
    ...
    require(
        ...
        "gfx950 FP4 GEMM hardware test requires a gfx950:xnack- physical device",
    )?;
    ...
    let execution = (|| -> Result<(), BoxError> {
        require(
            ...
            "gfx950 FP4 GEMM load observation changed",
        )?;
        ...
        let kernel = kernels.get(0).ok_or("runtime omitted gfx950 FP4 GEMM")?;
        require(
            kernels.len() == 1
                && resolutions.len() == 1
                && resolutions[0].executable_object() == executable_identity,
            "runtime resolved a substituted gfx950 FP4 GEMM kernel",
        )?;
        ...
        require(
            ...
            "gfx950 FP4 GEMM modified lhs",
        )?;
        require(
            rhs.read_after_synchronous_dispatch() == rhs_host,
            "gfx950 FP4 GEMM modified rhs",
        )?;
        verify_output(
    ...
    require(
        ...
        "reviewed HSA unload did not release the gfx950 FP4 GEMM executable",
    )?;
    execution
}

/// Runs a Rust-produced FP4 GEMM COV6 image on one gfx950 Wave64 workgroup.
///
...
/// FE2O3_GFX950_FP4_GEMM_HSACO=/absolute/canonical/gfx950-fp4-gemm.hsaco \
/// FE2O3_GFX950_FP4_GEMM_SHA256=<64-lowercase-hex-digits> \
...
#[cfg(feature = "hardware-test-hooks")]
#[test]
#[ignore = "non-authoritative: requires a Rust-produced digest-pinned gfx950:xnack- FP4 GEMM COV6 HSACO and MI350"]
fn gfx950_fp4_gemm_rust_cov6_runs_one_wave_and_matches_every_cpu_reference_output()
-> Result<(), BoxError> {
...
mod tests {
    ...
    #[test]
    fn e2m1_decoder_matches_all_canonical_exact_values() {
    ...
    #[test]
    fn deterministic_fp4_oracle_is_finite_axis_varying_and_complete() {
    ...
    fn output_validation_covers_every_element_and_both_canaries() {
        ...
        actual[CANARY_ELEMENTS + C_ELEMENTS - 1] += 1.0;
